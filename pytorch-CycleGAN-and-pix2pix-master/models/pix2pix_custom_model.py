...
# class STAGE1_Pix2PixModel(BaseModel):
#     def name(self):
#         return 'Pix2PixModel'
#
#     @staticmethod
...
#
# class STAGE2_Pix2PixModel(BaseModel):
#     def name(self):
#         return 'Pix2PixModel'
#
#     @staticmethod
...
class Pix2PixCustomModel(BaseModel):
    def name(self):
        return 'Custom Pix2PixModel'
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(which_model_netG='unet_256')
        if is_train:
            parser.add_argument('--lambda_A', type=float, default=100.0, help='weight for L1 loss G1-D1')
            parser.add_argument('--lambda_B', type=float, default=100.0, help='weight for L1 loss G2-D2')
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            use_sigmoid = opt.no_lsgan
            self.netD_A = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf,
                                          opt.which_model_netD,
                                          opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain,
                                          self.gpu_ids)
            self.netD_B = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf,
                                          opt.which_model_netD,
                                          opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, opt.init_gain,
                                          self.gpu_ids)
    ...
    def set_input(self, input):
        ...
        self.real_A = input['A' if AtoB else 'B'].to(self.device)
        self.real_B = input['B' if AtoB else 'A'].to(self.device)
        self.real_A_2 = input['A_2' if AtoB else 'B_2'].to(self.device)
        self.real_B_2 = input['B_2' if AtoB else 'A_2'].to(self.device)
    ...
    #resizing needed on real_A/ real_B --> discriminate role btwn D_A/D_B
    def backward_D_A(self):
        fake_AB = self.fake_AB_pool.query(torch.cat((self.real_A_2, self.fake_B_1), 1))
        real_AB = torch.cat((self.real_A_2, self.real_B_2), 1)
        self.loss_D_A = self.backward_D_basic(self.netD_A, real_AB, fake_AB)

    def backward_D_B(self):
        fake_AB = self.fake_AB_pool.query(torch.cat((self.real_A, self.fake_B_2), 1))
        real_AB = torch.cat((self.real_A, self.real_B), 1)
        self.loss_D_B = self.backward_D_basic(self.netD_B, real_AB, fake_AB)
